    ...
    def has_permission(self, request, view):
        ...
        if request.user.is_staff:
            if request.user.id == (int)(view.kwargs["analyst_pk"]):
                has_permission = True
                if view.action == "create" and "analyst" in request.data and (int)(request.data["analyst"]) != request.user.id:
                    has_permission = False
    ...
    def has_permission(self, request, view):
        ...
        if not request.user.is_staff:
            if request.user.id == (int)(view.kwargs["customer_pk"]):
                has_permission = True
                if view.action == "create" and "customer" in request.data and not (int)(request.data["customer"]) == request.user.id:
                    has_permission = False
